...
export default async function PitchDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const report = pitch.status === "COMPLETED" ? await getReport(id) : null;

  const submittedAt = new Date(pitch.createdAt).toLocaleString("en-US", {
  ...
  return (
    <main className="relative min-h-screen bg-surface pt-24 pb-20 px-4 md:px-6 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-4xl space-y-8">
        ...
        {/* Back */}
        <Link href="/dashboard" className="inline-flex items-center gap-1.5 text-sm text-on-surface-variant hover:text-on-surface transition-colors">
        ...
        <div className="relative bg-surface-lowest rounded-2xl shadow-ambient">
          ...
          <div className="p-6 md:p-8">
            ...
            {!report && (
              <div className="mt-5 pt-5 border-t border-surface-container">
                <p className="text-xs font-medium text-on-surface-variant tracking-wide uppercase mb-2">Your Idea</p>
        ...
        {pitch.status === "PROCESSING" && (
          <Card>
            ...
            <p className="text-xs text-on-surface-variant">
              Our AI agents are researching your idea across communities. You'll find the full report here once it's ready.
            </p>
          </Card>
        )}
        ...
        {/* ── Report sections (COMPLETED only) ── */}
        {report && (
          <>
            {/* ── Scorecard ── */}
            <section>
                  ...
                  <Card key={p.rank} className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <blockquote className="text-[12px] italic text-on-surface-variant border-l-2 pl-3" style={{ borderColor: "var(--color-primary-container)" }}>
                        &ldquo;{p.representative_quote}&rdquo;
                      </blockquote>
                    </div>
                  </Card>
                ))}
            ...
            {/* ── Sources ── */}
            <section>
              <SectionTitle subtitle="The real discussions and threads that informed every finding in this report.">
                Sources <span className="text-base font-normal text-on-surface-variant">({report.sources.length})</span>
              </SectionTitle>
              <Card>
                {keySources.length > 0 && (
                  <>
